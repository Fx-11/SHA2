...
bool isBigEnding() {  // 判断当前运行环境是否为大端
...
void Hash64() {  //单轮64次循环，结果保存于全局temph8数组
	for (int i = 0; i < 64; i++) {
...
void h8ToStr() {
	if (isBigEnd) {
		...
		struct hex2char h = { 0, 0 };  //将计算后的结果，即h8中存储的哈希值转换为字符串形式，并存储在result256中
		Byte* resultBegin = (Byte*)temph8;
...
char* HashStr(const char* message, int algorithm, bool isDebug, bit64 flaglengthb, bit64 flaglength) {
	...
	if (isBigEnd) {  //在大端环境中按次序即可---------------------------------------------------大大大-----------------------------------------------
		for (bit64 operate = 0; operate < howMany64; operate++) {  //前面的整64字节块
			...
			convert64to256();  //扩充到256字节
			Hash64();  //单轮64次循环计算
			for (short i = 0; i < 8; i++) {  //处理h8数组
				h8[i] += temph8[i];
				temph8[i] = h8[i];
			}
		}
		...
		for (bit64 operate = 0; operate < howMany64; operate++) {  //前面的整数次64字节块，为0时不执行
			...
			convert64to256();  //扩充到256字节
			Hash64();  //单轮64次循环计算
			for (short i = 0; i < 8; i++) {  //每个64字节块计算结束后处理h8哈希并复制给temph8，即单轮64次循环计算完成后更新h8哈希
				h8[i] += temph8[i];
				temph8[i] = h8[i];
			}
		}
...
char* HashFile(const char* path, int algorithm, bool isDebug) {
	...
	bit64 length8 = length << 3;  //总位数(需要填充的信息)
